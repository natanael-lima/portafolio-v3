...
export default function Projects() {
  return (
    <section id="project" className="mb-16">
      <h2 className="text-3xl font-bold text-gray-100 mb-4">Proyectos</h2>
      <div className="flex flex-col gap-8">
        {projects.map((project, index) => (
          <div key={index} className="flex flex-col md:flex-row w-full p-2 bg-gray-900/25 border border-gray-400/30 rounded-lg shadow-md">
            <div className="flex-1 flex flex-col justify-between p-4">
              ...
              <div className="mb-4 flex flex-wrap gap-2">
                {project.technologies.map((tech, i) => (
                  <span key={i} className="rounded-full bg-cyan-300/20 px-3 py-1 text-xs text-primary">
                    {tech}
                  </span>
                ))}
              </div>
